...
Idea: `polling_places` contains the lat & long of each place as well as the `PollingPlaceID`, so we can join it with the results table to get election result and use the lat & long to place it on a map.
...
Let's plot a function to zoom in on a city. Just a function choosing either `SYD` or `MEL`. Hint: Look at `?coord_map`.

My attempt:
```{r}
plot_city_my_attempt <- function(city){
  city_name <- if_else(city == "MEL", "Melbourne", "Sydney")
  
  polling_places %>%
    filter(DivisionNm == city_name) %>%
    select(PollingPlaceID, Latitude, Longitude) %>%
    inner_join(two_party_preferred_by_polling_place, by = "PollingPlaceID") %>%
    mutate(Winner = if_else(`Liberal/National Coalition Percentage` > 50, "Coalition", "ALP")) %>%
    ggplot(aes(x = Longitude, y = Latitude, colour = Winner)) +
...
plot_city_my_attempt("MEL") # should zoom in on Melbourne
```

The Solution:
```{r}
unabbrviate_city <- function(abbrev){
