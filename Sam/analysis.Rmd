...
`??thefunction` will show which installed package it belongs too.

```{r}
# Creae a new oject in this chunk
...
Need to test the model (against actual values). Firstly, let's just test against the data we used to construct the mode. To generate predictions given a model, use `predict(model, ...)`
...
This model is worse: it has a higher error rate (using the same erorr function).

This sort of error we calculated is called **training error **.

There are a few problems with the above model scenario. One is that people either survice or die: you can't 38 per cent survive. Likewise, you can't evaulate a model with categorical outputs using a continous function:
...
```{r}
titanic_training %>%
  mutate(predicted = sample(c(0,1), size = nrow(titanic_training), replace = TRUE, prob = c(1 - mean(.$Survived), mean(.$Survived))))
#             0.62                   0.38
...
This model above suffers from **overfitting**: it uses the names of people who dies in the dataset to determine who will die next. Because the dataset we're using to test is identical to what it was trained on, it appears to perform well. In realist, it's just repearing what was in the data already -- and it will perform terrrible on new data (with names not in the training data).

To get around this problem, we calculate the **test error** on *new data* 
-- data the model hasn't 'seen'. Typically we do this by partitioning up the data we have into a **training set** and a **test set**. Here is a common method:
...
# An anti-jon returns rows that DON'T match.
titanic_training_test <- 
  titanic_training %>%
    mutate(Name = factor(Name),
         Ticket = factor(Ticket),
         Cabin = factor(Cabin)) %>%
  # The 40% of the originbal dataset not in _train
...
3. How does this compare to an esitamte of the **test error**?
