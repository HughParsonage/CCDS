...
difference_and_swing_by_pollingplaceid <-
  two_party_prefered_by_polling_place %>%
  tbl_df %>%
  inner_join(two_party_prefered_by_polling_place_2013,by = "PollingPlaceID") %>%
  mutate(ppt_diff_Coalition_votes = `Liberal/National Coalition Percentage.y` - `Liberal/National Coalition Percentage.x`) %>%
  select(PollingPlaceID, contains("Percentage"), ppt_diff_Coalition_votes, Swing.x) %>%
  #Using {} allows you to use multiple columns after %>%
  # stopifnot(...) stops if any of the conditions are not met.
